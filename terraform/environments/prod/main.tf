...
locals {
  ...
  rds_allowed_cidr_blocks    = length(var.rds_allowed_cidr_blocks) > 0 ? var.rds_allowed_cidr_blocks : [var.vpc_cidr]
  rabbitmq_allowed_cidr_blocks = (
    length(var.rabbitmq_allowed_cidr_blocks) > 0 ? var.rabbitmq_allowed_cidr_blocks : [var.vpc_cidr]
  )
  redis_allowed_cidr_blocks = length(var.redis_allowed_cidr_blocks) > 0 ? var.redis_allowed_cidr_blocks : [var.vpc_cidr]
}
