...
2. Create one AWS IAM role trusted by GitHub OIDC for your fork's `main` branch.
...
1. Create or reuse the GitHub OIDC provider for `https://token.actions.githubusercontent.com`.
2. Create an IAM role whose trust policy allows `repo:<OWNER>/<REPO>:ref:refs/heads/main`.
3. Attach `.github/iam/terraform-prod-policy.json` as an inline policy on that role.
